...
import { selectedConferencePublications } from "../../data/conference-publications";
  ...
  return <main id="top">
    ...
    <div className="cv-shell">
      <aside className="sidebar">
        ...
        <p className="side-label">METHODS & TOOLS</p><ul className="plain-list"><li>ProVerif</li><li>AVISPA · Scyther</li><li>BAN · Expanded SVO</li><li>Security Benchmarking</li><li>MATLAB</li></ul>
        <p className="side-label">PROGRAMMING LANGUAGES</p><ul className="plain-list"><li>C / C++</li><li>C#</li><li>Java</li><li>Python</li><li>SQL</li><li>JavaScript / TypeScript</li></ul>
        <p className="side-label">FRAMEWORKS & WEB TECHNOLOGIES</p><ul className="plain-list"><li>MFC</li><li>React</li><li>Next.js</li><li>HTML / CSS</li></ul>
      </aside>

      <article className="cv-content">
        <section className="identity" id="profile"><p className="overline">ACADEMIC CURRICULUM VITAE</p><h1>고용호 <span>Yongho Ko</span></h1><p className="position">Ph.D. Candidate · Kookmin University</p><p className="summary">5G·6G 이동통신 보안을 중심으로 인증·키 합의 프로토콜, 정형화 검증, 양자내성암호 전환을 연구합니다. 표준 프로토콜의 보안 요구사항 분석을 실제 구현과 성능 평가로 확장합니다.</p><div className="metrics"><div><b>17</b><span>SCI Journal Articles</span></div><div><b>3</b><span>SCOPUS Papers</span></div><div><b>55</b><span>Research Projects</span></div><div><b>3</b><span>Granted Patents</span></div><div><b>1</b><span>Standard</span></div></div></section>

        <section className="cv-section"><h2>Education</h2><div className="entries"><div className="entry"><time>Present</time><div><h3>Ph.D. Candidate in Cyber Security</h3><p>Kookmin University, Seoul, Republic of Korea</p><small>Research: Mobile Network Security, Authentication Protocols, Formal Verification</small></div></div><div className="entry"><time>Master&apos;s</time><div><h3>Information Security</h3><p>Soonchunhyang University, Asan, Republic of Korea</p><small>Research: UAV Security and Authentication Protocols</small><a className="thesis-link" href={`${assetPrefix}/theses/espmu-efficient-security-protocol-military-uav.pdf`} target="_blank" rel="noreferrer"><span>MASTER&apos;S THESIS</span>미래전에 대비한 군수분야 UAV 환경을 위한 효율적인 보안 프로토콜 연구 ↗</a></div></div><div className="entry"><time>Bachelor&apos;s</time><div><h3>Information Security</h3><p>Soonchunhyang University, Asan, Republic of Korea</p><small>Research: Embedded Network Security, Firmware Reverse Engineering, and Digital Forensics</small><a className="thesis-link" href={`${assetPrefix}/papers/internet-router-firmware-update-vulnerability-analysis.pdf`} target="_blank" rel="noreferrer"><span>BACHELOR&apos;S THESIS</span>인터넷 공유기 펌웨어 업데이트 취약점 분석 ↗</a></div></div></div></section>
